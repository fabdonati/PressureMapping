...
switch loader
    ...
    case 'read_image_vtk2'
        ...
        fprintf(1,' ... parsing header of image %s (opened with Read_image_vtk2) ...\n',info.File);
        hPH = info;
        hPH.TransformMatrix = eye(3);
        hPH.CenterOfRotation= [0 0 0]; 
    case 'ICS'
    ...
    case 'load_untouch_nii'
        ...
        fprintf(1,' ... parsing header of image %s (opened with load_untouch_nii) ...\n',info.fileprefix);
        hPH = info;
        if (~bNotReloadHeader)
            ...
            % A more robust way in case ending in '.nii.gz'
            %[path file] = GetPath(info.fileprefix);
            %NIIfile = [path ls([info.fileprefix '*'])];
            hPH.TransformMatrix = eye(3);
            hPH.CenterOfRotation= [0 0 0];
    ...
    case 'ReadData3D'
        ...
        fprintf(1,' ... parsing header of image %s (opened with ReadData3D) ... \n',info.Filename);
    ...
    %               Magic: 'n+1 '
            ...
            case {'MHA','mha','.MHA','.mha'}
                ...
                %                  BitDepth: 16
                %                HeaderSize: 402
            case {'par','rec','.par','.rec','.PAR','.REC','PAR','REC'}
                hPH          = info;
                hPH.origin   = info.OffCentre;
                hPH.spacing  = info.Scales;   
                hPH.dim      = info.Dimensions(1:3);  
                hPH.TransformMatrix = eye(3);
                
%                             Filename: 'F:\PPE\Congenital\Archive_CL/kt8_pca_sparse_1.par'
%                    PatientName: 'ReconFrame'
%                ExaminationName: 'ReconFrame'
%                   ProtocolName: 'ReconFrame'
%            ExaminationDateTime: '2000.01.01 / 00:00:00'
%                  AcquisitionNr: 1
%               ReconstructionNr: 1
%                   ScanDuration: 0
%       MaxNumberOfCardiacPhases: 32
%              MaxNumberOfEchoes: 1
%     MaxNumberOfSlicesLocations: 50
%            MaxNumberOfDynamics: 1
%               MaxNumberOfMixes: 1
%                PatientPosition: 'HFS'
%           PreparationDirection: 'AP'
%                      Technique: 'TFE'
%                 ScanResolution: [160 88]
%                       ScanMode: '3D'
%                 RepetitionTime: 5.0300
%                            Fov: [203.5000 370 115.5000]
%                  WaterFatShift: 0.4900
%                     Angulation: [0 0 0]
%                      OffCentre: [16.7600 -10.1000 6.7800]
%               FlowCompensation: 0
%                  Presaturation: 0
%          PhaseEncodingVelocity: [0 0 150]
%                            Mtc: 0
%                           Spir: 0
%                      EpiFactor: 1
%                    DynamicScan: 0
...
%     MaxNumberOfGradientOrients: 2
%             NumberOfLabelTypes: 0
%                  HeaderComment: {1x9 cell}
%               SliceInformation: [1x3200 struct]
%           ImageInformationTags: [1x39 struct]
%                     Dimensions: [160 160 50 64]
%                         Scales: [2.3120 2.3120 2.3100]
%                    FilenameREC: 'F:\PPE\Congenital\Archive_CL\kt8_pca_sparse_1.rec'
%                       BitDepth: 16
%                         Format: '.par'
            case 'DICOM'
                hPH          = info;
                hPH.origin   = info.ImagePositionPatient;
                if isfield(info,'PixelDimensions')
                    hPH.spacing  = info.PixelDimensions;   
                else if isfield(info,'PixelSpacing')
                        hPH.spacing = info.PixelSpacing;
                        % this might be only a 2D spacing
                        if numel(hPH.spacing)==2
                            if isfield(info,'SpacingBetweenSlices')
                                hPH.spacing(3) = info.SpacingBetweenSlices;
                            else
                                fprintf('WARNING! No info of voxel size in Z\n');
                            end
                        end
                    else
                        fprintf('WARNING! no info on voxel size\n');
                    end
                end
                ...
                if isfield(info,'Dimensions')
                    ...
                    hPH.dim(1)   = info.Width;
                    hPH.dim(2)   = info.Height;
                    hPH.dim(3)   = 1;
                    fprintf('WARNING! need to reconstruct the number of slices\n')
                end
                    
                % Need to debug the correct reading of this information:
                 C1 = info.ImageOrientationPatient(1:3);
                 C2 = info.ImageOrientationPatient(4:6);
                 C3 = cross(C1,C2);
                 hPH.TransformMatrix = [C1 C2 C3];
