%> @file extractparrecinfoormationfromfile.m
%> 
...
%>
%> function [outputdirectionmatrix, outputorigin] = extractdirections (filename)
...
%> header
%> 
%> Usage : [M,O] = extractdirections (myparfile)
%>
%> @author Nicolas Toussaint nicolas.toussaint@kcl.ac.uk

function [outputdirections, outputorigin] = extractparrecinformationfromfile (filename)

header = par_read_header(filename);

sliceangulations = header.Angulation;
offcenter        = header.OffCentre;

slicethickness = header.SliceInformation(1).SliceThickness + header.SliceInformation(1).SliceGap;
numberofslices = size(header.SliceInformation,2);

%spacing          = [ header.SliceInformation(1).PixelSpacing slicethickness];
%dimensions       = [ header.SliceInformation(1).ReconResolution numberofslices ];
%sliceorientation =   header.SliceInformation(1).SliceOrientation;
spacing          = header.Scales(1:3);
dimensions       = header.Dimensions(1:3);
sliceorientation = header.SliceInformation(1).SliceOrientation;

[outputdirections, outputorigin] = extractparrecinformation (sliceorientation, sliceangulations, offcenter, spacing, dimensions);

%error of approximately half voxel in outputorigin, only for the 1st and second component:
outputorigin(1:2) = outputorigin(1:2) - (header.Scales(1:2)' / 2.0);
